...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #LSDCoinTEST2\r");
            Send(hSocket, "WHO #LSDCoinTEST2\r");
        } else {
            // randomly join #LSDCoin00-#LSDCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #LSDCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #LSDCoin%02d\r", channel_number).c_str());
        }
