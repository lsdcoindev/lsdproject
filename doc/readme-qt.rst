LSDCoin-qt: Qt4 GUI for LSDCoin
===============================
...
Alternatively, install Qt Creator and open the `LSDCoin-qt.pro` file.

An executable named `LSDCoin-qt` will be built.
...
A warning for people using the *static binary* version of LSDCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of LSDCoin is linked against libdb5.3.
...
installed causes LSDCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
